...
bool Heap::extend_heap(size_t size)
{
    ...
    ADD_TO_VOID_P(last_page, VMMGR_PAGE_SIZE);

    bool status = true;

    size_t number_of_pages_done;
    for (number_of_pages_done = 0; number_of_pages_done < desired_pages; number_of_pages_done++, ADD_TO_VOID_P(last_page, VMMGR_PAGE_SIZE))
    {
...
void Heap::free(void *addr)
{
    ...
    as well! */
    SUBTRACT_FROM_VOID_P(addr, sizeof(size_t)); // Go to the actual beginning of the allocated space.
...
void *Heap::realloc(void *ptr, size_t new_size)
{
    ...
    void *tmp = ptr;
    SUBTRACT_FROM_VOID_P(tmp, sizeof(size_t));
